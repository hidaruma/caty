...
class Run(Internal):
    ...
    command run 
            ...
            refers python:caty.command.fit.Run;
    """
...
class List(Builtin):
    ...
    command list :: void -> void
        ...
        updates stream
        uses interpreter
        refers python:caty.command.fit.List;
    """
    def execute(self):
        d = self.behaviors.opendir('/')
        r = []
...
class ClearReport(Internal):
    ...
    command clean {"all": boolean, "all-apps":boolean} [string?] :: void -> void
        uses [pub, behaviors]
        reads env
        refers python:caty.command.fit.ClearReport;
    """
...
class SendReport(Builtin):
    ...
    command report {"all": boolean} :: void -> void
        reads [env, pub]
        refers python:caty.command.fit.SendReport;
    """
    def setup(self, opts):
        self._all = opts.all
