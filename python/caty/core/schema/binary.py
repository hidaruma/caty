...
class BinarySchema(ScalarSchema):
    ...
    def __init__(self, *args, **kwds):
        ...
        if self.minLength > self.maxLength and self.maxLength is not None:
            raise JsonSchemaError(i18n.get('minLength($min) is longer than maxLength($max)', min=self.minLength, max=self.maxLength))
    def _validate(self, value):
        ...
        if not isinstance(value, str):
            raise JsonSchemaError(ro.i18n.get(u'value should be $type', type='binary'), value, '')
        if self.minLength != None and self.minLength > len(value):
            raise JsonSchemaError(ro.i18n.get(u'string length should longer than $len', len=self.minLength))
        if self.maxLength != None and self.maxLength < len(value):
            raise JsonSchemaError(ro.i18n.get(u'string length should shorter than $len', len=self.maxLength))
