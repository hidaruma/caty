...
def initialize(config):
    ...
    storagemodule.initialize(config['conf'])
    return StorageModuleWrapper(storagemodule, config['conf'])

def storage_class_factory(mod):
...
class StorageModuleWrapper(Facility):
    def __init__(self, mod, conf, mode=READ):
        self._mod = mod
        self._storage_class = storage_class_factory(mod)
        self._conf = conf
        self._conn = None
        self._storage = None
    ...
    def __call__(self, collection_name, app_name=''):
        ...
        smw = self.clone()
        smw._storage = smw._storage_class(self._conn, self._schema_module, collection_name, app_name, self.application)
